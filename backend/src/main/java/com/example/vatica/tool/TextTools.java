...
public final class TextTools {
    ...
    @Tool(name = "calculator", description = "计算数学表达式并返回数值结果。支持 + - * / 四则运算、括号、小数、百分号、负数。"
            ...
            + "只做数值运算，不解析自然语言。")
    public String calculate(@ToolParam(name = "expression", description = "数学表达式，长度不超过 200 字符，如 \"(3+4)*2\"",
    ...
    @Tool(name = "text_stats", description = "统计一段文本的中文字数、总字符数（不含空白）、行数、段落数。"
            ...
            + "只做统计，不修改文本内容。")
    public String textStats(@ToolParam(name = "text", description = "待统计的文本内容", required = true) String text) {
        if (text == null) {
            text = "";
        }
        if (text.isBlank()) {
