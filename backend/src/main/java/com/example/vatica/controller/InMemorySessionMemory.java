 ...
 * （{@code maxSessions}）按 LRU 淘汰防内存无限增长。
 *
 * <p>纯 POJO，不依赖 Spring，单测可直接 {@code new}；synchronized 保证并发安全
 * （虚拟线程场景多请求并发写同一会话也不丢消息）。
 */
public final class InMemorySessionMemory implements SessionMemory {
    ...
    public InMemorySessionMemory(int maxMessages, int maxSessions, int maxChars) {
        if (maxMessages <= 0) {
    ...
    synchronized void restore(String sessionId, List<ConversationMessage> messages) {
        ...
        trim(bucket);
        sessions.put(keyOf(sessionId), bucket);
    }
